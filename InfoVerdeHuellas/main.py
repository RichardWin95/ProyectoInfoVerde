...
from models import db,BasTipoMaquina, BasOficiona, BasCargo, BasResponsable, BasFactorEmision, BasMuestra,\
    AlchemyEncoder, json,TrnDetalleMuestra,BasUser
...
def login():
    ...
    if request.method == "POST" and login.validate():
        ...
        user = BasUser.query.filter_by(user_name=username).first()
...
@app.route("/")
def index():
    return render_template('indexHuellas.html')
@app.route("/maquina")
def machine():
...
def menu():
    return render_template('index.html')
@app.route('/api/huellas/consumoE')
...
def produccionCO2e():
    ...
    response = jsonify(response)
    return response
@app.route('/api/huellas/detailSample')
def dataSetConsumo():
    lista = list()
    query = db.engine.execute(
